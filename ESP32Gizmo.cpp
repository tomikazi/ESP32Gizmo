#include <ESP32Gizmo.h>
#include <ESP32GizmoHTML.h>
#include <FS.h>
#include <SPIFFS.h>
...
#define MAX_WILL_TOPIC_SIZE     128
static char defaultWillTopic[MAX_ANNOUNCE_MESSAGE_SIZE];
#define MAX_WILL_MESSAGE_SIZE   128
...
void listDir(WebServer *server, const char *path) {
    ...
    while (file) {
        ...
        char name[48];
        name[0] = '\0';
        strncat(name, file.name(), 47);
        Serial.printf("%s\t%d\n", name, file.size());
        snprintf(line, 127, "%-32s %8d<br>", name, file.size());
...
void ESPGizmo::setupWiFi() {
    ...
    strncpy(hostname, name, MAX_SSID_SIZE);
